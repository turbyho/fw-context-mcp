---
name: release
description: Use ONLY when the user asks to commit, release, bump version, publish to PyPI, or make a GitHub release for the fw-context-mcp project. Automates the full release pipeline: bump version, run tests, commit with Conventional Commits format, tag, push to both remotes, create GitHub release, build package, and upload to PyPI.
---

# /release — Commit & Release Automation

Automates commits and releases with strict conformance to project conventions.
...
### Release notes rules
- Title is `v<version> — <short title>` (no emoji, no markdown in `--title`)
- Overview section is mandatory
- If no features, omit `### Features`; same for fixes
