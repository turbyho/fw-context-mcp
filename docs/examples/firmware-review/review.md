...
|---|--------|---------|-------------|------|
| 1 | BOARD_V1 cleanup | 16 files | Removal of target, MODEM_A, sensors, bootloader | ✅ |
| 2 | BLE messaging subsystem | radio_msg.cpp/h, radio_msg_manager.cpp/h, cbor_msg.cpp/h, msg_manager.cpp/h | New BLE protocol (SDKv3), framing, CRC, CBOR | ✅ |
| 3 | radio_driver.cpp/h | radio_driver.cpp (+363), radio_driver.h (+68) | SDKv3 char, MTU adaptation, chunked TX | ✅ |
| 4 | net_msg.cpp/h | net_msg.cpp (+718), net_msg.h (+135) | KeyFormatV3, PANEL_COM, async open/cancel | ✅ |
| 5 | config_manager/fram | config_manager.cpp (+621), config_store.cpp (+237) | Key rotation, FRAM migration v1→v2, generic layout | ✅ |
| 6 | panel_keypad.cpp/h + radio_cmd | panel_keypad.cpp (+272), radio_cmd.cpp (+189) | Multipin/multishipment, panel_com, SDKv2 guard | ✅ |
| 7 | record_writer | record_writer.cpp (+172) | Snapshot logic, dispatch_num_shipments, conditional set_slot | ✅ |
| 8 | NCbor contracts | lib/cbor_gen/generation/ (20+ files) | key_format_v2, key_format_v3, radio_token, panel_com, msg_reservation | ✅ |
| 9 | app_manager, batt, download, panel_manager, uart_drv, sd_drv, runtime_data | 14 files | BOARD_V1 removal, RadioMsgManager integration, SESSION_KEY critical level | ✅ |
| 10 | Configuration and build | app_config.h, mbed_app.json, profiles, build_app.py, CI | NTP dual-pool, BLE parameters, restrict_size, SESSION_KEY_INIT | ✅ |

---

## Structural analysis — key architectural changes

### 1. BOARD_V1_MODEM_A removal — **CLEAN** ✅

fw-context found that all references to `BOARD_V1`, `MODEM_A`, `MODEM_VENDOR`, and `TARGET_BOARD_V1` are completely removed:
- `search_content("BOARD_V1", project_only=True)` → **0 results** (except a comment in `panel_manager.cpp`)
- `search_content("MODEM_A")` → **0 results**
- `search_content("MODEM_VENDOR")` → **0 results**
- 7 files physically deleted (modem_board_v1.cpp, sensors_BOARD_V1.cpp, panel_modbus.cpp/h, MODEM_A driver, BOARD_V1 bootloader)
- `find_dead_code` → no orphaned functions from the removal
- Minor issue: 3 dead `#include <nrf_nvmc.h>` (rs485.cpp, sd_drv.cpp, radio_driver.cpp)

> **💡 fw-context insight — verifying complete removal:**
> `search_content("BOARD_V1", project_only=True)` searched all application files
> through the FTS5 index, in a single call. A traditional `grep -r "BOARD_V1"`
...
AppManager::_periodic_app_task()
  ├── RadioMsgManager::process_cons()  — RX framing, CRC, CBOR decode
  ├── RadioMsgManager::process_prod()  — TX framing, CBOR encode, chunked write
  └── RadioMsgManager::process_timers()
       ├── _radio_msgs[TOKEN]  — RadioMsgToken (active)
       ├── _radio_msgs[STATE]  — RadioMsgState (WIP, not yet implemented)
       └── _radio_msgs[DEFAULT] — fallback
```

Inheritance: `MsgManager → RadioMsgManager`, `CborMsg → RadioMsg → {Token, State, Default}`

> **💡 fw-context insight — mapping new architecture:**
> `get_file_map("radio_msg.cpp")` and `get_file_map("radio_msg_manager.cpp")` gave a
> structured table of contents for the new files instead of dumping 800+ raw
> lines. `get_inheritance_chain("device::MsgManager", transitive=true)` and
> `get_method_overrides("device::RadioMsg::process_prod_specialized")` revealed the
...
- **File:line** — `src/radio_driver.cpp:564-568`
- **Root cause:** `_rx_mail_box.try_alloc()` allocates a slot. When `_rx_mail_box.put(p_msg_rx)` fails (mailbox full), `_rx_mail_box.free(p_msg_rx)` is missing.
  ...
  if (status != osOK) {
      APP_ERROR("rx put err:%d", (int)status);
...
- **Evidence:** `get_source("device::RADIO::onDataWritten")` — `try_alloc()` at line 547, `break` at line 567 without `free()`.
- **Fix:** Add `_rx_mail_box.free(p_msg_rx);` before `break;` on line 567.

> **💡 fw-context insight — finding memory leaks with precise extents:**
> `get_source("device::RADIO::onDataWritten")` returned **only** the function
> body, from the opening `{` to the closing `}`, through libclang exact
> extents. A manual `read radio_driver.cpp offset=540 limit=40` would require
...
- **File:line** — `src/radio_driver.cpp:730`
- **Root cause:** Function declares `radio_error_t err` (line 721), but `APP_ERROR` on line 730 uses `(int) error` — variable `error` does not exist in this scope.

  ```cpp
  radio_error_t err = _ble.gap().updateConnectionParameters(...);  // line 721
  ...
  APP_ERROR("updateConnectionParameters error, %d", (int) error); // line 730
  ```

- **Runtime impact:** Compilation error, or — if `error` is pulled from an outer scope — the log prints an incorrect value.
- **Evidence:** `get_source("device::RADIO::requestConnParamsUpdate")` — `err` declared line 721, `error` used line 730. `lookup_symbol(exact=true, "error")` confirmed that `error` is **not** a global variable in application code. `lookup_symbol` found only local variables in other functions, and the mbed `error()` function.
...
- **File:line** — `src/config_manager.cpp` (`_initialize_keys()`)
...
- **Runtime impact:** PIN operations fail until the server sends new keys via the key_format_v3 endpoint.
...
- **File:line** — `src/net_msg.cpp:4387-4414`
- **Root cause:** CBOR decode succeeds, but the code never reads `panel_com_cons_s.map_shp` and `panel_com_cons_s.map_st`. The code receives the response, but never uses the status (`st`) and shipment ID (`shp`) from the response.
- **Runtime impact:** The code does not recognize a server error state. The PANEL_COM response is effectively "fire and forget".
...
#### 6. Missing `frame_length` validation in `RadioMsgManager::process_cons()`

- **File:line** — `radio_msg_manager.cpp:168`
...
> `get_source("device::RadioMsgManager::process_cons")` returned the complete
...
- **File:line** — `radio_msg_manager.cpp:251-260`
...
- **File:line** — `radio_msg_manager.cpp:428-434` vs `app_manager.cpp:482-485`
...
> functions. `find_callers("device::RadioMsgManager::process_timeout")`
> returned `_thread_app_func`. `find_callers("device::RadioMsgManager::process_cons")`
...
- **File:line** — `src/record_writer.cpp:172-212` (`dispatch_open_slot`)
...
#### 10. SESSION_KEY critical level change — `NONE` → `BOTH`

- **File:line** — `src/runtime_data.cpp:45`
- **Root cause:** `CFG_INVALID_SKEY` previously `operation_typ = NONE`, now `BOTH` — an invalid SESSION_KEY blocks both IN and OUT operations.
- **Runtime impact:** On SESSION_KEY degradation during operation (expired provisioning key, corrupted configuration), the box stops working for customers.
...
- **File:line** — `src/radio_token.cpp:32`
...
#### 12. `PANEL_COM` uses `SocketType::NORMAL` — potential blocking

- **File:line** — `src/net_msg.cpp` (default `_is_socket_ok()`)
- **Root cause:** PANEL_COM messages compete with normal messages (telemetry, journal) on the NORMAL socket. `ModemMsgReservation` by contrast uses the HIGH socket.
- **Runtime impact:** COMPLETE/CANCEL operations may wait if the NORMAL socket is processing another message.
- **Fix:** Consider using the HIGH socket for PANEL_COM.

### 🔵 Info (7)

#### 13. `RadioMsgState::process_prod_specialized()` — unreachable code

- **File:line** — `radio_msg.cpp:866-882`
- **Description:** `return RadioMsg::ProdSpecializedStatus::ERROR;` on line 867 precedes CBOR encode code. `RadioMsgState` is not yet implemented.

#### 14. Unused methods in header — `_process_prod()`, `_release_prod_buffer()`

- **File:line** — `radio_msg_manager.h:107,112`
...
- **File:line** — `src/config_manager.cpp`
- **Description:** `find_callers("device::ConfigManager::get_last_key")` → zero callers. Method should be removed.
...
#### 18. Unused variable `elapsed_time` in `panel_keypad.cpp`

- **File:line** — `src/panel_keypad.cpp:543`
- **Description:** `uint32_t elapsed_time = _runtime_data.get_uptime_ms();` — value never used.

#### 19. Inconsistent types in `radio_mail_rx_data_t`

- **File:line** — `src/radio_driver.h:28-33`
- **Description:** `_rx_data` is `char[]`, but `radio_mail_tx_data_t._tx_data` is `uint8_t[]`. The reviewer recommends unifying on `uint8_t`.
...
| R7 | `SESSION_KEY_INIT_BASE64` — mandatory CI macro, build fails without it | **Low** — CI is configured |
| R8 | `RadioMsgToken` is the only active handler, `RadioMsgState` WIP | **Low** — expected by design |
...
|-------|-------|-----------|
| BOARD_V1/MODEM_A removal | 7 deleted files, 16 cleaned up, clean build | ✅ |
| New BLE messaging subsystem | 8 new files, parallel to ModemMsgManager | ✅ (with reservations) |
| SDKv3 BLE characteristic + chunked TX | RADIO +363 lines, MTU adaptation, new UUID | ✅ (2 critical bugs) |
| KeyFormatV3 protocol (key rotation) | New endpoints, 2-slot model, commit flow | ⚠️ Breaking storage change |
| PANEL_COM endpoint | New communication channel for slot operations | ⚠️ Response ignored |
...
| 🟡 Warning | 10 | Key storage breaking change, ignored PANEL_COM response, debug restrict_size, missing frame_length validation, race condition, lost snapshot, SESSION_KEY BOTH |
| 🔵 Info | 7 | Dead code, unused variables, inconsistent types, pre-existing bugs |

**Overall impression:** The changes represent a significant architectural shift — transition to SDKv3 BLE stack, new key management (V3), removal of BOARD_V1 target. The code is predominantly well-structured and follows existing conventions. **2 critical bugs must be fixed before release.** 10 warnings require attention — especially the breaking change in key storage layout and the ignored PANEL_COM response.
