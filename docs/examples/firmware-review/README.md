...
A real-world demonstration of **fw-context** applied to a comprehensive code
review of an embedded C/C++ firmware project. This case study shows what
fw-context enables, how to use each tool effectively, and the quantitative
token savings compared to traditional `grep` + `read` workflows.
...
|---------|------|-------------------------------------|
| Memory leak in BLE RX handler | `get_source` | Exact function extents showed the missing `free()` — `grep` can't find missing code |
| `error` vs `err` typo | `get_source` + `lookup_symbol` | Proved the variable doesn't exist in scope — `grep` returns hundreds of false positives |
...
| Dead code after migration | `find_callers` | Zero callers confirmed — `grep` would find the definition and look like it's used |
...
**9 out of 19 findings** could not have been discovered with traditional `grep` + `read` workflows.
...
tokens** — the review would be **unrealizable in a single session**. The
biggest wins come from tools that have no manual equivalent:
`find_all_callers_recursive` (428× savings), `find_dead_code` (400×),
and `find_references` (320×).

Full breakdown in [`token-savings.md`](token-savings.md).
...
The actual code review deliverable — 19 findings across 10 review areas,
annotated with **💡 fw-context insight** callouts explaining which tool was
used and why. See what a deep, fw-context-powered review looks like in
practice.

**Highlights:**
- How `get_source` with libclang exact extents caught a memory leak that
  manual `read` would likely miss (Finding #1)
- How `lookup_symbol` proved a variable doesn't exist in application scope —
  something `grep` can't do (Finding #2)
- How `find_callers` on two functions from different thread contexts revealed
  a race condition invisible to single-file reading (Finding #8)

### [`methodology.md`](methodology.md) — How It Was Done

The complete methodology behind the review — tool selection guide, per-agent
breakdown, anti-patterns avoided, and recommendations for getting the most
out of fw-context.
...
The quantitative analysis of token savings — per-tool breakdown from 10×
(`get_source`) to 428× (`find_all_callers_recursive`), hidden savings from
false positive elimination and ifdef-filtered content, and the bottom-line
conclusion: **108× fewer tokens, 99.1% effective savings**.
...
3. **`get_symbol_context` over `get_source`** — it returns body + callers +
   callees in one call. Saves ~30% of round-trips.
...
6. **fw-context is for C/C++ only** — JSON, YAML, Python, and git operations
   still use standard tools. Don't force it.

---

*The raw source files for this case study are preserved in
