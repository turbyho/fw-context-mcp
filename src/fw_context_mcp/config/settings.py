...
def derive_project_id(root: Path) -> str:
    """Return the project ID, auto-generating one if not yet set.

    The ID is stored in the ``[project]`` section of the shared project
    config (``.fw-context/config.toml``).  It is stable, team-shared via
    git, and independent of filesystem location.

    If no ID exists yet (project not initialized), one is auto-generated
    and persisted so subsequent calls are stable.  This allows the MCP
    server and tools to start in any project without requiring
    ``fw-context init`` first — enabling self-bootstrapping where the
    agent detects ``no_index`` via ``get_active_build()`` and runs
    ``fw-context index`` via bash automatically.
    """
    ...
    try:
        _write_project_id(root, new_id)
        log.info("Auto-generated project ID %s for %s", new_id, root)
    except Exception:
        log.exception("Failed to persist auto-generated project ID for %s", root)
    return new_id
