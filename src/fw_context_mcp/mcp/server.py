...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "A NAME THAT MEANS SEVERAL SYMBOLS — a common method name lives in\n"
        "many classes.  `read`, `write`, `get` and `size` each match dozens\n"
        "of symbols in a firmware project.\n"
        "ONE-BODY tools — get_source, get_symbol_context, explain_symbol —\n"
        "answer for one symbol and add:\n"
        "• `candidates` — a row per symbol that the name matched, with\n"
        "  `qualified_name`, `class`, `kind`, `file`, `line` and\n"
        "  `signature`.  Pick from it and ask again with that\n"
        "  `qualified_name`.  Read `class` first: it is what tells two\n"
        "  same-name methods apart.\n"
        "• `candidates_total` — how many symbols the name matches\n"
        "  ALTOGETHER.  When it is larger than the list, `candidates` holds\n"
        "  the most referenced ones; lookup_symbol(name, exact=True) lists\n"
        "  them ALL and its `offset` pages through them.  The two orders\n"
        "  differ, thus start that listing at the top — it is not a\n"
        "  continuation of `candidates`.\n"
        "• `ambiguous_warning` — one sentence naming the symbol answered.\n"
        "LIST tools — find_all_callers_recursive, find_callees_recursive,\n"
        "find_call_path, find_callers, find_references — answer for ALL of\n"
        "them.  A `warning` row comes first and names the symbols, and each\n"
        "result carries `target_qualified_name`, which tells the symbol it\n"
        "belongs to.\n"
        "• Read `target_qualified_name` before you say who calls what.\n"
        "  Without it a caller of one class reads as a caller of the other.\n"
        "• Give the full qualified name (`ClassB::probe`) to ask about one\n"
        "  symbol.  An exact qualified name always beats a bare sibling.\n"
        "• lookup_symbol lists every match with its `class`, and its\n"
        "  `offset` parameter pages through a name that many classes\n"
        "  share.\n\n"
        ...
        "builds on two axes: `variant` is the board, `image` is the program.\n"
        ...
        "  `variant_images` and `active_variant`.  Call list_variants for one\n"
        "  row per indexed build, with its own config_hash, board, counts and\n"
        "  memory map.  Both read the INDEX and not only the config, thus a\n"
        "  build indexed with --variant is listed even when config.toml no\n"
        "  longer declares it.\n"
        "• Pass `variant` and `image` to every call that TAKES them.  Every\n"
        "  tool that answers about code takes them, with two exceptions:\n"
        "  smart_search and semantic_search take neither, and they answer\n"
        "  for the active build.  An argument a tool does not declare is an\n"
        "  error that names it.\n"
        "• To learn whether a symbol is in the bootloader TOO, ask twice, once\n"
        "  per image.  Two plain answers beat one blended answer.\n\n"
        "READING PAST THE FIRST PAGE — eight tools take an `offset` and lead\n"
        ...
        "  row comes before it when the index is stale, and so does the row\n"
        "  that reports a query FTS5 could not parse.\n"
        "• When `more` is true, call again with offset=<offset + shown>.\n"
        "  The `hint` spells out that call.\n"
        "• Do NOT conclude \"that is all of them\" from one page.  Read\n"
        "  `total` — a hot function can have hundreds of call sites, and a\n"
        "  common name such as `read` can name hundreds of symbols.\n"
        "• Each order is stable, thus two pages never overlap and never skip\n"
        "  a row.\n"
        ...
        "Source: get_source, get_symbol_context, get_file_map, explain_symbol, read_file.\n"
         ...
         "  • ONE QUERY ANSWERS FOR ONE BUILD.  Every tool that answers about\n"
         "    code takes variant=<name> and (sysbuild) image=<name>, except\n"
         "    smart_search and semantic_search, which take neither.\n"
         ...
         "  • A variant that holds several images also fails closed: name the\n"
         "    image, because each image is a separate program.\n"
         "  • variant=\"*\" is REFUSED.  To learn whether a symbol is in two\n"
         "    builds, ask twice, once per build.  No answer merges builds, and\n"
         "    no row carries a variant/image label.\n"
