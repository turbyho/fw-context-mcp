...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "A NAME THAT MEANS SEVERAL SYMBOLS — two classes can each hold a\n"
        "method of the same name, thus a bare `probe` names both\n"
        "`ClassA::probe` and `ClassB::probe`.\n"
        "Every tool that takes a symbol name handles this, in one of two\n"
        "shapes.\n"
        ...
        "answer for one symbol and add the key `ambiguous_warning`, which\n"
        "names the one they chose and lists the others.\n"
        "• Read `target_qualified_name` before you say who calls what.\n"
        "  Without it a caller of one class reads as a caller of the other.\n"
        "• `ambiguous_warning` means you are reading one of several bodies.\n"
        "  Ask again with the qualified name before you conclude.\n"
