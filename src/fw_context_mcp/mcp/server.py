...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "READING PAST THE FIRST PAGE — find_callers and find_references lead\n"
        "with a page notice: {total, offset, shown, more, hint}.  It is\n"
        "ALWAYS there, thus a full page never leaves you guessing whether\n"
        "more exists.\n"
        "• When `more` is true, call again with offset=<offset + shown>.\n"
        "  The `hint` spells out that call.\n"
        "• Do NOT conclude \"that is all the callers\" from one page.  Read\n"
        "  `total` — a hot function can have hundreds.\n"
        "• The order is stable (file, then line), thus two pages never\n"
        "  overlap and never skip a row.\n\n"
