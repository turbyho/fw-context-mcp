...
# Rate limiting — 50 req/s burst 20 (must be in http context)
...
def write_nginx_config(domain: str, proxy_port: int = 8000) -> Path:
    ...
    try:
        ...
        print(f"Failed to write {config_path} — write it manually:", file=__import__("sys").stderr)
        print(config_text)
    return config_path
...
def enable_nginx_site() -> None:
    ...
    try:
        ...
        print(f"Failed to enable site — run: sudo ln -s {src} {dst}", file=__import__("sys").stderr)
