...
def _accumulate_openai_sse(resp: httpx.Response) -> str:
    ...
    Raises:
        OllamaError: If no content was extracted from the SSE stream.
    """
    chunks: list[str] = []
    ...
    if not chunks:
        raise OllamaError("SSE stream contained no content chunks")
    return "".join(chunks)
...
def _stream_openai_chat(
    ...
    *,
    ...
    Raises:
        OllamaError: On network, API, or parse failure.
    """
    payload["stream"] = True
    t0 = time.monotonic()
    chunks: list[str] = []
    ...
    if not chunks:
        raise OllamaError("Streaming chat API returned no content chunks")
